import { Strmbuf } from "./strmbuf";
import { Coap_MTypes, Coap_Codes, isKnownMessageCode, Coap_Options } from "./coapcodes";
import { AddressInfo } from "net";
import {Coap_Message, Coap_Option} from "./coap_message"

const PROTOCOL_VERSION = 1;
const MAX_TOKEN_LENGTH = 8;


export function coap_decode(rawbuffer:Buffer,rinfo:AddressInfo):Coap_Message|undefined {
	let buffer:Strmbuf=new Strmbuf(rawbuffer);
	//Decode the Message Header which must have a length of exactly 4 bytes


	//Decode the header values
	let encodedHeader:number|undefined = buffer.readInt(4);
	...
	if (tokenLength>0) token = buffer.readBuf(tokenLength);

	//Handle empty message (ignore everything but the first 4 bytes)
	if (messageCode == Coap_Codes.EMPTY) {
		let msg:Coap_Message= {
			rinfo,
			messageType:messageType,
			messageCode:Coap_Codes.EMPTY,
			messageID, 
			tok:token, 
	...
	if (isKnownMessageCode(msg.messageCode)==undefined) {
		return msg;
	}

try{
	//Decode and set the options
	if (buffer.avb > 0)  setOptions(msg, buffer);
} catch(e) {
	return msg;
}	
	...
	if (msg.messageType!=Coap_MTypes.RST){
		expand_common(msg);
	}
	return msg;
}
...
function opt_as_be_int(opt:Coap_Option):number|undefined{
	if (opt.val.length>4) return undefined;
	let res=0;
	for (let i=0; i<opt.val.length; i++) res=(res <<8 ) | (opt.val[i] & 0x0FF);
	return  res;
}

function opt_as_le_int(opt:Coap_Option):number|undefined {
	if (opt.val.length>4) return  undefined;
	let res=0;
	for (let i=opt.val.length-1; i>=0; i--) res=(res <<8 ) | (opt.val[i] & 0x0FF);
	return  res;
}

function setOptions(msg:Coap_Message,  buffer:Strmbuf) {
	...
	while(firstByte != 0xFF && buffer.avb >= 0) {
		...
		if (msg.opt==undefined) msg.opt=new Array<Coap_Option>();
...
function expand_common(msg:Coap_Message){
	if (msg.opt) for (let opt of msg.opt){
		if (opt.num==Coap_Options.URI_PATH) {
			if (!msg.uri_path) msg.uri_path='/'+opt.val.toString('utf8');
			else msg.uri_path=msg.uri_path+'/'+opt.val.toString('utf8');
		} else if (opt.num==Coap_Options.COIOT_DEVID) {
			msg.coiot_devidrev=opt.val.toString('utf8');
			let dp=msg.coiot_devidrev.split('#');
			msg.coiot_dev=dp[0];
			msg.coiot_id=dp[1];
			msg.coiot_rev=dp[2];
		} else if (opt.num==Coap_Options.COIOT_STATUS_SERIAL) {
			msg.coiot_stat_serial=0; let sh=0;
